...
from fetchers.journals_fetcher import fetch_papers_for_journals, JOURNAL_FEEDS
...
class JournalsSource(BaseSource):
    ...
    DEFAULT_JOURNALS = [
        "nature", "science",
        "nature_food", "nature_water", "nature_climate", "nature_cities",
        "science_adv", "one_earth", "earths_future", "cell_rep_sus",
    ]
    ...
    def __init__(self, source_args: dict, llm_config: LLMConfig, common_config: CommonConfig):
        ...
        # 过滤掉不认识的 key
        self.journals = [j for j in self.journals if j in JOURNAL_FEEDS]
    ...
    def add_arguments(parser: argparse.ArgumentParser):
        parser.add_argument(
            ...
            help="[Journals] 期刊 key 列表，可选: " + ", ".join(JOURNAL_FEEDS.keys()),
        )
        parser.add_argument(
            "--jr_max_results", type=int, default=30,
            help="[Journals] 每本期刊最多抓取的条目数",
        )
        parser.add_argument(
...
直接返回上述 JSON 格式，无需任何额外解释。"""
    ...
    def get_section_header(self) -> str:
        names = [JOURNAL_FEEDS[k][0] for k in self.journals[:3] if k in JOURNAL_FEEDS]
        hint = ", ".join(names)
